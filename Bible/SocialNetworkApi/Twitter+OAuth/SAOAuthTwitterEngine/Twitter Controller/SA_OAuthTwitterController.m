...
@synthesize engine = _engine, delegate = _delegate, navigationBar = _navBar, orientation = _orientation,_blockerView;
...
- (void) loadView {
	...
	self._blockerView = [[[UIView alloc] initWithFrame: CGRectMake(304, 482, 200, 60)] autorelease];
	self._blockerView.backgroundColor = [UIColor colorWithWhite: 0.0 alpha: 0.8];
	self._blockerView.alpha = 0.0;
	self._blockerView.clipsToBounds = YES;
	if ([self._blockerView.layer respondsToSelector: @selector(setCornerRadius:)]) [(id) self._blockerView.layer setCornerRadius: 10];
	
	UILabel	*label = [[[UILabel alloc] initWithFrame: CGRectMake(0, 5, self._blockerView.bounds.size.width, 15)] autorelease];
	label.text = NSLocalizedString(@"Please Wait…", nil);
	label.backgroundColor = [UIColor clearColor];
	label.textColor = [UIColor whiteColor];
	label.textAlignment = UITextAlignmentCenter;
	label.font = [UIFont boldSystemFontOfSize: 15];
	[self._blockerView addSubview: label];
	
	UIActivityIndicatorView	*spinner = [[[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle: UIActivityIndicatorViewStyleWhite] autorelease];
	
	spinner.center = CGPointMake(self._blockerView.bounds.size.width / 2, self._blockerView.bounds.size.height / 2 + 10);
	[self._blockerView addSubview: spinner];
	[self.view addSubview: self._blockerView];
...
- (void) didRotateFromInterfaceOrientation: (UIInterfaceOrientation) fromInterfaceOrientation {
	...
	self._blockerView.center = CGPointMake(self.view.bounds.size.width / 2, self.view.bounds.size.height / 2);
